...
#define AWAIT_AND_ASSERT_TOPIC(sber, expected_topic)                           \
	TEST_ASSERT_EQUAL_STRING(                                              \
		expected_topic,                                                \
		channel_gettopic(load_getchan(subscriber_await(sber))))
...
static void* task(void* arg)
{
	...
	publish(broker_search(broker, "response.subscribed"), "%d", id);

	AWAIT_AND_ASSERT_TOPIC(sber, "request.traffic");
	for (;;)
		if (load || (load = subscriber_poll(sber))) {
			if (strcmp("request.join",
			           channel_gettopic(load_getchan(load))) == 0)
				break;
			TEST_ASSERT_GREATER_THAN_INT(N_THREADS, atoi(load));
			TEST_ASSERT_LESS_OR_EQUAL_INT(0, atoi(load));
			counters[id][atoi(load)]++;
			load = NULL;
		} else if (i < N_NOTIFS) {
			publish(broker_search(
					broker, topics_out[rand() % n_out]),
			        "%d", id);
			++i;
		} else if (!done) {
			printf("[%ld] Publishing to \"response.done\".\n", id);
			done = 1;
			publish(broker_search(broker, "response.done"), "%ld",
			        id);
		} else {
			load = subscriber_await(sber);
		}
	publish(broker_search(broker, "response.join"), "%ld", id);
	return NULL;
}
...
TEST(broker_extra, should_be_safe_from_race_conditions)
{
	...
	publish(broker_search(broker, "request.traffic"), "%p", NULL);

	for (int i = 0; i < N_THREADS; i++)
		AWAIT_AND_ASSERT_TOPIC(sber, "response.done");
	publish(broker_search(broker, "request.join"), "%p", NULL);

	for (int i = 0; i < N_THREADS; i++)
		pthread_join(thid[atoi(subscriber_await(sber))], NULL);
