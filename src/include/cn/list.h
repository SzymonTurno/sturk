...
#define CN_LIST(name, type)                                                   \
	name { union { name* next; struct CnUnnode node; } u; type data; }

#else /* __STRICT_ANSI__ */

#define CN_LIST(name, type)                                                   \
	name { union { name* next; struct CnUnnode node; }; type data; }
...
#define cn_list_data(list)                                                    \
	({                                                                     \
		__typeof__(list) _list = (list);                               \
									       \
		CN_ENSURE(_list, CN_ECODES.null_param);                        \
		&(_list)->data;                                                \
	})
...
#define cn_list_hand(listp, pos)                                              \
	({                                                                     \
		...
		long long _pos = (pos);                                        \
									       \
		CN_ENSURE(_listp, CN_ECODES.null_param);                       \
...
#define _CN_LIST_ITER(type, _i, listp, pos, ...)                              \
	type** _i = (listp); *_i; _i = cn_list_hand(_i, (pos))

#ifdef __STRICT_ANSI__

#define _CN_LIST_INS(list, entry, pos, ...)                                   \
	((void*)cn_unnode_ins((struct CnUnnode*)(list),                        \
		(struct CnUnnode*)(entry), (pos)))
...
#define _CN_LIST_INS(list, entry, pos, ...)                                   \
	({                                                                     \
		...
		__typeof__(&list) _i = cn_list_hand(&_list, (pos));            \
									       \
		CN_ENSURE(_entry, CN_ECODES.null_param);                       \
		_entry->next = *_i;                                            \
		*_i = _entry;                                                  \
		_list;                                                         \
	})
