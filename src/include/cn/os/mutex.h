...
enum CnMutexProtocol {
	CN_MUTEX_POLICY_NONE         = CN_MUTEX_BF(POLICY, 0),
	CN_MUTEX_POLICY_PRIO_INHERIT = CN_MUTEX_BF(POLICY, 1),
	_CN_MUTEX_POLICY_DELIM       = CN_MUTEX_BF(POLICY, 8)
};

enum CnMutexType {
	CN_MUTEX_TYPE_NONE      = CN_MUTEX_BF(TYPE, 0),
	CN_MUTEX_TYPE_RECURSIVE = CN_MUTEX_BF(TYPE, 1),
	_CN_MUTEX_TYPE_DELIM    = CN_MUTEX_BF(TYPE, 16)
};
